    ...
    <main class="container flex-grow p-4 sm:p-6">
      ...
      <div class="mb-6 flex flex-col justify-between gap-y-1 sm:flex-row sm:gap-y-0">
        ...
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item">
            <a href="#">Fuel Type List</a>
          </li>
        </ol>
      </div>
      <!-- Page Title Ends -->

      <!-- Fuel Type List Starts -->
      <div class="space-y-4">
        <!-- Fuel Type Header Starts -->
        <div class="flex flex-col items-center justify-between gap-y-4 md:flex-row md:gap-y-0">
          ...
          <div class="flex w-full items-center justify-between gap-x-4 md:w-auto">
                ...
                <div class="dropdown-content w-72 !overflow-visible">
                  <ul class="dropdown-list space-y-4 p-4">
                    <li class="dropdown-list-item">
                      <h2 class="my-1 text-sm font-medium">Fuel Type</h2>
            ...
            <a class="btn btn-primary" href="{{route("fuel-type.create")}}" role="button">
              <i data-feather="plus" height="1rem" width="1rem"></i>
              <span class="hidden sm:inline-block">Add Fuel Type </span>
            </a>
          </div>
          <!-- Customer Action Ends -->
        </div>
        <!-- Fuel Type Header Ends -->

        <!-- Fuel Type Table Starts -->
        <div class="table-responsive whitespace-nowrap rounded-primary">
          <table class="table">
            <thead>
              <tr>
                <th class="w-[40%] uppercase">Sl No</th>
                <th class="w-[40%] uppercase">Fuel Type name</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td> <p class="truncate text-xs text-slate-500 dark:text-slate-400">
                       {{ Illuminate\Support\Str::limit($fuel_type->description, $limit = 20, $end = '....') }}
                       {{-- <a href="{{ route('fuel_type.edit', ['id' => $fuel_type->id]) }}">Read More</a> --}}
                   </p>
                  </td>
                  ...
                  <td>
                    <div class="badge badge-soft-success">
                        {{ $fuel_type->status == 0 ? 'Draft' : '' }}
                        {{ $fuel_type->status == 1 ? 'Inactive' : '' }}
                        {{ $fuel_type->status == 2 ? 'Active' : '' }}
                  </td>
                  <td>
                      ...
                      <div class="dropdown" data-placement="bottom-start">
                        ...
                        <div class="dropdown-content w-40">
                          <ul class="dropdown-list">
                            ...
                            <li class="dropdown-list-item">
                              <a href="{{route('fuel-type.edit',['fuel_type'=>$fuel_type->id])}}" class="dropdown-link">
                            ...
                            <li class="dropdown-list-item">
                              <a  class="dropdown-link">
                                ...
                                <form action="{{route('fuel-type.destroy',['fuel_type'=>$fuel_type->id])}}" method="post">
                                    @method('DELETE')
                                    @csrf
                                    <input type="hidden" value="{{$fuel_type->id}}" name="fuel_type">
                                    <input type="submit" value="Delete"  onclick="return confirm('Do you want to Delete fuel_type !')">
                                </form>
                              </a>
                            </li>
                          </ul>
        ...
        <!-- fuel_type Table Ends -->

        <!-- fuel_type Pagination Starts -->
